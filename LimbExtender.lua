...
function LimbExtender:_applyLimbs(player, char, limb)
    ...
    if self._settings.ESP and self._ESP then
        ...
        if not tracked then
            task_spawn(function()
                local attempts = 0
                while self._running and not self._ESP:Track(char) and attempts < 30 do
                    task_wait(0.1)
                    attempts = attempts + 1
                end
            end)
...
function LimbExtender:_processDirtyWork()
    if self._working then return end  
    self._working = true
    self._workScheduled = false       

    local processESP      = self._dirtyESP
    local processRestart  = self._dirtyRestart
    local processCosmetic = self._dirtyCosmetic
...
function LimbExtender:Start()
    ...
    if not self._working and not self._workScheduled then
        self._workScheduled = true
...
function LimbExtender:Set(key, value)
    ...
    if self._running and not self._working and not self._workScheduled then
        self._workScheduled = true
        task_spawn(function() self:_processDirtyWork() end)
    end
end
