		...
		local function characterAdded(character)
			if character then
				...
				if playerData then
					...
					if rawSettings.TEAM_CHECK and localPlayer.Team ~= nil and player.Team == localPlayer.Team then
						return
					end
					...
					task.spawn(function()
						...
						if humanoid and humanoid.Health > 0 then
							partStreamable:Observe(function(part, trove)
								if not limbExtenderData[rawSettings.TARGET_LIMB] then
									...
									pcall(function()
										...
										mt.__index = function(Self, Key)
											if tostring(Self) == rawSettings.TARGET_LIMB and tostring(Key) == "Size" then
												return size
											end
											return old(Self, Key)
										end
	...
	if not limbExtenderData["indexBypass"] then
		...
		pcall(function()
			...
			for _, gcItem in ipairs(getgc(true)) do
				if typeof(gcItem) ~= "table" then continue end
