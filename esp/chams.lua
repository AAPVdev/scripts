...
local function hasBypassCapabilities()
    local required = {
        "hookfunction", "newcclosure", "checkcaller", "sethiddenproperty",
        "getrawmetatable", "setreadonly", "clonefunction", "getactorthreads",
        "run_on_thread"
    }
    for _, name in ipairs(required) do
        if type(_G[name] or getgenv()[name]) ~= "function" then
            return false
        end
    end
    return true
...
if hasBypassCapabilities() and not getgenv().__InstanceBypassLoaded then
